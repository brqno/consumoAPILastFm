...
def ensure_database_exists() -> None:
    """Cria o banco configurado quando ele ainda não existe no servidor."""
    master_connection = urllib.parse.quote_plus(_odbc_connection_string("master"))
    master_engine = create_engine(
        f"mssql+pyodbc:///?odbc_connect={master_connection}",
    ...
    try:
        with master_engine.connect() as conn:
            ...
            if exists is None:
                ...
                conn.exec_driver_sql(f"CREATE DATABASE [{database_identifier}]")
                print(f"[LOAD] Banco criado: {DATABASE}")
    finally:
